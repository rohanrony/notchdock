...
struct CalendarSettingsView: View {
                    ...
                    SectionCard(title: "Thresholds", subtitle: "Customize when event details appear.") {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            HStack(alignment: .top, spacing: 10) {
                                ...
                                Text("NotchDock automatically switches to the next event 10 minutes before a meeting starts if the current one is still ongoing.")
                                    .font(.system(size: 11))
                                    .foregroundColor(.secondary)
                                    .fixedSize(horizontal: false, vertical: true)
                            }
    ...
    var permissionDescription: String {
        if viewModel.isAuthorized {
            return "NotchDock has access to your calendar events."
        }
        switch viewModel.authorizationStatus {
        case .denied:
    ...
    private var privacyNote: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("NotchDock is fully local and never connects to any server. We request calendar access solely to display your schedule in the notch; your events are read directly from the system database and are never stored or transmitted.")
...
struct CalendarModule: NotchDockExtension {
    var id: String = "com.notchdock.calendar"
